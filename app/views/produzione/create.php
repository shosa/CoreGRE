...
<!-- Main Form -->
<form id="produzioneForm" action="<?= $this->url('/produzione/store') ?>" method="POST" class="space-y-6">
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            <div class="space-y-2">
                ...
                <select id="month" name="month" required
                    class="border w-full rounded-lg border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white focus:border-primary focus:ring-primary">
                    <?php foreach ($monthNames as $monthName): ?>
                        <option value="<?= $monthName ?>" <?= $monthName === strtoupper($currentMonth) ? 'selected' : '' ?>>
                            <?= $monthName ?>
                        </option>
                    <?php endforeach; ?>
                </select>
            ...
            <div class="space-y-2">
                ...
                <select id="day" name="day" required
                    class="border w-full rounded-lg border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white focus:border-primary focus:ring-primary">
                    <?php for ($i = 1; $i <= 31; $i++): ?>
                        <option value="<?= $i ?>" <?= $i == (int) $currentDay ? 'selected' : '' ?>>
                            <?= sprintf('%02d', $i) ?>
                        </option>
                    <?php endfor; ?>
                </select>
            ...
            <!-- Year (Hidden, auto-filled) -->
            <div class="space-y-2">
                <label for="year" class="block text-sm font-medium text-gray-700 dark:text-gray-300">
                    Anno
                </label>
                <input type="number" id="year" name="year" value="<?= date('Y') ?>" readonly
                    class="border w-full rounded-lg border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white bg-gray-50">
            </div>
        </div>
    </div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <div class="space-y-4">
                <div class="space-y-2">
                    ...
                    <input type="number" id="manovia1" name="manovia1" min="0" step="1" value=""
                        class="border w-full rounded-lg border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white focus:border-blue-500 focus:ring-blue-500"
            ...
            <div class="space-y-4">
                <div class="space-y-2">
                    ...
                    <input type="number" id="manovia2" name="manovia2" min="0" step="1" value=""
                        class="border w-full rounded-lg border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white focus:border-blue-500 focus:ring-blue-500"
                        onchange="calculateTotals()">
                </div>
                <div class="space-y-2">
            ...
            <?php for ($i = 1; $i <= 5; $i++): ?>
                <div class="space-y-4">
                    <div class="space-y-2">
                        ...
                        <input type="number" id="orlatura<?= $i ?>" name="orlatura<?= $i ?>" min="0" step="1" value=""
                            class="border w-full rounded-lg border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white focus:border-green-500 focus:ring-green-500"
                            onchange="calculateTotals()">
                    </div>
                    <div class="space-y-2">
                        <label for="orlaturanote<?= $i ?>"
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <div class="space-y-4">
                <div class="space-y-2">
                    ...
                    <input type="number" id="taglio1" name="taglio1" min="0" step="1" value=""
                        class="border w-full rounded-lg border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white focus:border-orange-500 focus:ring-orange-500"
                        onchange="calculateTotals()">
                </div>
                <div class="space-y-2">
            ...
            <div class="space-y-4">
                <div class="space-y-2">
                    ...
                    <input type="number" id="taglio2" name="taglio2" min="0" step="1" value=""
                        class="border w-full rounded-lg border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white focus:border-orange-500 focus:ring-orange-500"
                        onchange="calculateTotals()">
                </div>
                <div class="space-y-2">
    ...
    <!-- Totals Display -->
    <div
        class="rounded-2xl border border-gray-200 bg-gradient-to-r from-purple-50 to-pink-50 dark:from-purple-900/20 dark:to-pink-900/20 p-6 shadow-lg dark:border-gray-800 backdrop-blur-sm">
        <h3 class="text-lg font-semibold text-gray-900 dark:text-white mb-6 flex items-center">
        ...
        <div class="grid grid-cols-1 md:grid-cols-4 gap-6">
            <div class="text-center p-4 bg-white dark:bg-gray-800 rounded-xl shadow-sm">
                ...
                <div id="totalMontaggio" class="text-2xl font-bold text-blue-600 dark:text-blue-400">0</div>
            </div>

            <div class="text-center p-4 bg-white dark:bg-gray-800 rounded-xl shadow-sm">
                <div class="text-sm font-medium text-gray-600 dark:text-gray-400 mb-2">Orlatura</div>
                <div id="totalOrlatura" class="text-2xl font-bold text-green-600 dark:text-green-400">0</div>
            </div>

            <div class="text-center p-4 bg-white dark:bg-gray-800 rounded-xl shadow-sm">
                <div class="text-sm font-medium text-gray-600 dark:text-gray-400 mb-2">Taglio</div>
                <div id="totalTaglio" class="text-2xl font-bold text-orange-600 dark:text-orange-400">0</div>
            </div>

            <div
                class="text-center p-4 bg-gradient-to-r from-purple-100 to-pink-100 dark:from-purple-800/50 dark:to-pink-800/50 rounded-xl shadow-sm border-2 border-dashed border-purple-200 dark:border-purple-600">
                <div class="text-sm font-medium text-gray-600 dark:text-gray-400 mb-2">Totale</div>
                <div id="totalGenerale" class="text-2xl font-bold text-purple-600 dark:text-purple-400">0</div>
            </div>
        </div>
    </div>
...
<script>
    ...
    document.getElementById('produzioneForm').addEventListener('submit', function (e) {
        ...
        if (!month || !day || !year) {
            e.preventDefault();
            showAlert('Per favore seleziona una data valida', 'error');
            return;
        }
        ...
        if (window.WebgreModals) {
            ...
            if (confirm('Sei sicuro di voler salvare questi dati di produzione?')) {
                e.target.submit();
            }
        }
    });

    // Carica URL parameters se presenti
    document.addEventListener('DOMContentLoaded', function () {
        ...
        if (month) {
            document.getElementById('month').value = month.toUpperCase();
        }
